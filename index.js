...
function createConversation(personaName) {
...
async function ask(conversation, prompt) {
    ...
    conversation.messages.push({ role: "assistant", content: hinglishResponse });

    console.log(`${conversation.persona.label}:`, hinglishResponse);
    return hinglishResponse;
}


const personaName = process.argv[2] || "hitesh";
const prompt = process.argv[3] || "sir, DSA phele karu ya Development?";

const conversation = createConversation(personaName);
ask(conversation, prompt).catch(console.error);